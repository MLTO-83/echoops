...
cd /var/www/portavi || cd /root/portavi
...
mkdir -p /var/www/portavi/scripts

echo -e "${GREEN}Copying updated process-ai-jobs.ts...${NC}"
cp -f /root/portavi/scripts/process-ai-jobs.ts /var/www/portavi/scripts/

echo -e "${GREEN}Copying test script for repositories with spaces...${NC}"
cp -f /root/portavi/scripts/test-repo-with-spaces.js /var/www/portavi/scripts/

echo -e "${GREEN}Copying utility to reset failed jobs with spaces...${NC}"
cp -f /root/portavi/scripts/reset-failed-jobs-with-spaces.js /var/www/portavi/scripts/
...
if ! command -v npx &> /dev/null; then
  ...
  tsc /var/www/portavi/scripts/process-ai-jobs.ts --esModuleInterop
else
  npx tsc /var/www/portavi/scripts/process-ai-jobs.ts --esModuleInterop
fi

# Check if compilation was successful
if [ -f "/var/www/portavi/scripts/process-ai-jobs.js" ]; then
...
chmod +x /var/www/portavi/scripts/process-ai-jobs.js
...
echo -e "${GREEN}cd /var/www/portavi && node scripts/test-ai-job-processor.js${NC}"
echo -e "${GREEN}To specifically test repositories with spaces in their names:${NC}"
echo -e "${GREEN}cd /var/www/portavi && node scripts/test-repo-with-spaces.js${NC}"
...
echo -e "${GREEN}   tail -f /var/www/portavi/scripts/process-ai-jobs.log${NC}"
echo -e "${GREEN}3. Reset a specific failed job:${NC}"
echo -e "${GREEN}   cd /var/www/portavi && node scripts/reset-specific-job.js <job-id>${NC}"
echo -e "${GREEN}4. Reset all failed jobs:${NC}"
echo -e "${GREEN}   cd /var/www/portavi && node scripts/reset-failed-jobs.js${NC}"
echo -e "${GREEN}5. Reset all failed jobs with spaces in repository names:${NC}"
echo -e "${GREEN}   cd /var/www/portavi && node scripts/reset-failed-jobs-with-spaces.js${NC}"
